...
begin
  DataModule1.FDQueryReport.Active := false; // отключаем запрос, если он был включен
...
begin
     // создаём фрейм отчёта
   BAllData.Click;
end;

procedure TFrReports.Image1Click(Sender: TObject);
   // этапы построения полного отчёта
begin
    // удаляем временную таблицу
   BDropTab.Click;
 // создаём новую таблицу, включаем DataSet
   BDB.Click;
  // импортируем данные в таблицы базы данных
   BImportData.Click;
  // создаём рисунки графиков
   BPicture.Click;
...
procedure TFrReports.ImportDatePipeSection;
 ...
 i : Integer;
begin
// добавляем данные расчёта в таблицу БД секции колонны бурильных труб фрейм отчёта
...
procedure TFrReports.ImportDateTitle;
 // сохраняем в базу данных информацию тз таблицы StringGridDate
...
begin
  ...
  // Сохраняем информацию перзонализации расчёта
...
begin
 // проекция профиля скважины
  ...
  // конвертируем Bitmap в PNG и сохраняем
  ...
  // диаграмма состовляющих формулы расчёта веса бурильной колонны
  bmp := TBitmap.Create;
